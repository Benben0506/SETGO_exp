...
class DirectoryDataset(Dataset):
    def __init__(self, root, path, image_set, transform, target_transform):
        ...
        if os.path.exists(join(self.dir, "labels")):
            ...
            self.img_files = np.array([x[:-3] for x in self.img_files])
            self.label_files = np.array([x[:-3] for x in self.label_files])

            c = np.intersect1d(self.label_files, self.img_files)
            #print('c', c)

            self.img_files = np.array([x+'jpg' for x in c])
            self.label_files = np.array([x+'png' for x in c])
...
class ContrastiveSegDataset(Dataset):
    def __init__(self,
        ...
        output_dir = f"{pytorch_data_dir}nns"
        feature_cache_file = f"{output_dir}/nns_{cfg.model_type}_{image_set}_{crop_type}_{cfg.res}.npz"
