...
class TrainingExperiment:
    ...
    def submit(self, registered_dataset):
        ...
        output_training_weights = OutputFileDatasetConfig(name="training_weights", source="/mnt/data/training_weights/")
        ...
        train_step = PythonScriptStep(
            ...
            outputs=[output_training_weights.as_mount()],
            compute_target=self._cluster_name,
            runconfig=gpu_run_config,
        )

        supervised_train_step = PythonScriptStep(
            name="Train1",
            script_name="train_segmentation_1.py",
            source_directory=".",
            inputs=[
                input_dataset.as_mount(path_on_compute="/mnt/data/input_dataset"),
                output_dataset.as_input(name="precomputed_knns").as_mount(path_on_compute="/mnt/data/output_dataset"),
                output_training_weights.as_input(name="training_weights").as_mount(path_on_compute="/mnt/data/training_weights")
            ],
            compute_target=self._cluster_name,
            runconfig=gpu_run_config,
        )

        pipeline = Pipeline(workspace=self._workspace, steps=[precompute_knns_step, train_step, supervised_train_step])
        pipeline.validate()
        return pipeline
